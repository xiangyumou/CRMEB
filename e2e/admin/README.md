...
| `specs/decor.spec.ts`           | 店铺装修（新版）：新建 → 改页面标题 → 保存草稿（商城 404）→ 发布（商城读到）→ 带未保存修改再发布（先存后发，第 2 版）→ 回滚到第 1 版成为第 3 版、商城立即读到旧内容、草稿不动；只读查看历史版本；预览先存草稿，iframe 里是凭预览令牌读到的草稿，无令牌仍 404；未发布页不可设为首页，发布后设为首页 → `/api/v1/pages/home` 即是它（DECOR-008、DECOR-011、DECOR-012、DECOR-014） |
