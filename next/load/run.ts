 ...
 *     memory limits and tuning of `deploy/next/compose.yml` (swap disabled, so
...
async function main() {
  ...
  if (!existsSync(path.join(webDir, '.next/standalone/apps/web/server.js'))) {
    throw new Error(
      'apps/web/.next is missing — run `pnpm turbo run build --filter @shop/web` from next/ first',
    );
  }
  if (!existsSync(path.join(workerDir, 'dist/main.js'))) {
    throw new Error(
      'apps/worker/dist is missing — run `pnpm turbo run build --filter @shop/worker` from next/ first',
    );
  }
  const common = {
    NODE_ENV: 'production',
