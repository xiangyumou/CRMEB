 ...
 * REL-007, STAB-001).
 *
 * The publish rules live in one script, which `next.yml` calls. What this
...
export function readPipeline(
  ...
  need(
    /deploy\/next\/rehearsal\/drill\.sh/.test(workflow),
    'does not run the deploy rehearsal `deploy/next/rehearsal/drill.sh`',
  );
