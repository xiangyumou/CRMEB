...
const SOURCE_ROOTS = [
  path.join(nextRoot, 'apps/web/src'),
  path.join(nextRoot, 'apps/web/app'),
...
const DENY_LISTS: readonly DenyListFile[] = [
  {
    file: /^next\/packages\/contracts\/src\/diy\/removed\.ts$/,
  ...
  {
    file: /^next\/packages\/core\/src\/diy\/diy\.test\.ts$/,
    words: ['砍价', '秒杀'],
    why: 'the cleanDiyData test: it asserts a saved page containing those components comes back without them',
  },
  {
    file: /^next\/packages\/contracts\/src\/system\/system\.role\.contract\.ts$/,
    words: ['秒杀'],
...
export const retiredFeatures = defineCheck(
  'retired',
  'no retired feature reappears in next/ or the uni-app API layer',
  () => {
