...
| `specs/order.spec.ts`           | 快递发货缺物流公司/运单号被 refine 挡住；发货 → 确认收货 → 订单真的流转 + 审计留痕（CR-15-k 已修，真跑）；已发完的单不再出现「发货」（单独跑时也经界面发货，不走服务层）                                                                                                                                        | K-SEC-A9、X7                                                                                                 |
| `specs/notification.spec.ts`    | 退款申请通知：效果行 → 处理器 → 有 `refund:request:read` 的管理员收件箱恰好一条、重复 `notify` 不加条、标已读计数；无此权限的管理员计数不变、碰别人的消息 404；发送记录「已发送」；模板页改站内信标题后下一条用新标题；SSE 无会话 401、有会话收到具名 `notification` 事件；铃铛两条 `test.fail`（**CR-32-k2**） | USER-003、NOTIF-002、NOTIF-004；AUDIT「未覆盖」通知一条；`contracts` 守卫 `UNCONTRACTED`（SSE 自行校验会话） |
| `specs/wechat-oa.spec.ts`       | 对 `@shop/testing` 的假公众号服务：保存草稿不碰微信；未配置时发布被拒且零外呼；配置后发布，微信收到的菜单树与输入逐字段相等、用的是它签发的 token、审计留痕；线上菜单被拒后页面显示 errcode；草稿被拒无处可见（`test.fail`，**CR-33-k2**）                                                                      | K-SEC-A9；AUDIT「未覆盖」公众号一条；「不碰真实网关」                                                        |
| `specs/presale.spec.ts`         | 界面新建预售活动（规格行、日期、金额）→ 商城列表与详情按预售价可买；对话框改标题后规格价不丢（CR-3-d2）；暂停即下架、启用即恢复；草稿按 id 仍可读（`test.fail`，**CR-34-k2**）                                                                                                                                  | STOCK-004 的前台一侧；AUDIT「未覆盖」预售一条                                                                |
| `specs/statistics.spec.ts`      | 营业额、支付订单数、客单价按 `DEFINITIONS.md` 用本文件的 SQL 独立算出并逐字相等；订单页「订单销售额」「订单量」与之一致；商品统计导出带 BOM、表头固定、名为 `=1+1…` 的商品被 `'` 化解                                                                                                                           | CONSOLE-003 同一属性（统计导出那份实现）；AUDIT「未覆盖」统计一条                                            |
| `specs/refund.spec.ts`          | 审核界面根本没有金额字段；拒绝必须填理由；同意后状态离开 `applied` 且效果账本恰好一行                                                                                                                                                                                                                           | K-SEC-R7（HTTP 边界那半）                                                                                    |

## `test.fail` 与待办

套件里没有 `test.fixme`。有四条 `test.fail`，每一条都是**本套件查出来的缺陷**，指向一个 CR；缺陷修好那天 Playwright 会报「Expected to fail, but passed」，删掉那一行即可：

| 用例                                                              | CR           | 缺陷                                                                                     |
| ----------------------------------------------------------------- | ------------ | ---------------------------------------------------------------------------------------- |
| `notification.spec.ts`「the bell shows a pushed notification」    | **CR-32-k2** | SSE 发的是具名事件 `event: notification`，铃铛只挂了 `onmessage`，推送到了浏览器又被丢掉 |
| `notification.spec.ts`「the bell shows the unread inbox on load」 | **CR-32-k2** | 没有任何后台页面调用收件箱接口，铃铛每次刷新都是空的                                     |
| `wechat-oa.spec.ts`「WeChat refusing a new draft…」               | **CR-33-k2** | 草稿发布被微信拒绝，错误写在草稿行上，而页面只读线上菜单的错误                           |
| `presale.spec.ts`「a draft is not on the storefront…」            | **CR-34-k2** | 商城预售详情不按状态过滤，草稿的标题、价格、规格按 id 可读                               |

每条 `test.fail` 都先断言前置条件成立（推送确实到了浏览器、微信确实答了 502、草稿确实建成），所以它们失败的原因只能是 CR 描述的那一个。

本套件仍不覆盖的：

| 待办                             | 说明                                                                |
| -------------------------------- | ------------------------------------------------------------------- |
| 门店/客服台（`/api/v1/staff/*`） | **CR-14-k**（staff 路由对 staff 用户 403）                          |
| 客户、注册、短信验证码           | 商城侧流程，属于 I 的 uni-app 端到端套件，不在后台套件里            |
| 拼团、运费模板、CMS              | 页面已合并（D、F2），这一轮没有写 spec                              |
