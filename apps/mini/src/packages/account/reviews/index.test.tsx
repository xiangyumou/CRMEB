...
describe('我的评价', () => {
  it('shows a review with its reply, and one waiting for review neutrally', async () => {
    signIn();
    serveApi({
      'GET /api/v1/me/reviews': () => ({
        body: page([review, { ...review, id: '52', content: '还不错', status: 'pending' }]),
      }),
    });
    await renderPage(<MyReviewsPage />);

    expect(await screen.findByText('包装很严实。')).toBeTruthy();
    expect(screen.getAllByText('谢谢支持')).toHaveLength(2);
    expect(screen.getAllByRole('img', { name: '4 星' })).toHaveLength(2);
