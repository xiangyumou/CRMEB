...
/**
 * `GET /api/v1/diy/pages/user-center` as HTTP.
 *
...
async function read() {
  ...
  const response = await GET(get('/api/v1/diy/pages/user-center'));
...
describe('GET /api/v1/diy/pages/user-center', () => {
  it('answers the built-in default without a session', async () => {
    ...
    expect(body).toMatchObject({ id: null, kind: 'user_center', name: '个人中心' });
    expect(body.content).toEqual(USER_CENTER_DEFAULT_VALUE);
  ...
  it('answers the published page once there is one', async () => {
    ...
    // An operator who kept only the member header: one real node out of the default.
    const content = Object.fromEntries(
      Object.entries(USER_CENTER_DEFAULT_VALUE).filter(
        ([, node]) => (node as { name?: string }).name === 'member',
      ),
    );
    ...
    const { response, body } = await read();
    expect(response.status).toBe(200);
    expect(body).toMatchObject({ id: page.id, kind: 'user_center', title: '我的' });
