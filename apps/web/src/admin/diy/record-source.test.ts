...
import { createDiyDataSource, isClaimableNow, isRunningNow } from './record-source';
...
const source = () => createDiyDataSource({ now: () => NOW });
...
describe('拼团', () => {
  ...
  it('asks for active activities and maps title, image, price and seats', async () => {
    ...
    const result = await source().list('combination', { page: 1, pageSize: 10 });
  ...
  it('resolves stored activity ids through the detail route', async () => {
    ...
    expect(await source().resolve('combination', ['1'])).toEqual([
      {
        id: '1',
        name: '三人成团 · 坚果礼盒',
        image: 'https://cdn.example.com/p/11.jpg',
