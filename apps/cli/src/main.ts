 ...
 *
 * Output is JSON on stdout; failures are a line on stderr and a non-zero exit.
 */
...
令牌在后台「设置 → API 令牌」新建。也可以用环境变量 SHOP_ORIGIN、SHOP_TOKEN。`;
...
async function run(argv: string[]): Promise<number> {
  ...
  switch (command) {
    case 'login': {
      ...
      const token = (values.token === '-' ? await readStdin() : values.token).trim();
      if (!token.startsWith('shp_')) throw new UsageError('令牌应以 shp_ 开头');
      const config = { origin: normaliseOrigin(values.origin), token };
      const me = await callOperation(config, 'auth.adminMe');
      if (!me.ok) {
...
run(process.argv.slice(2)).then(
  ...
  (error: unknown) => {
    ...
    process.stderr.write(`${known ? text : `出错了：${text}`}\n`);
