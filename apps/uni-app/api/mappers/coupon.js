// coupon DTOs → the 优惠券 view models.
//
// Contract: packages/contracts/src/coupon/coupon.storefront.contract.ts
//
// Two different shapes share one page family:
...
import { toId, toInt, money, text, list, mapList, unixSeconds, pageDate } from './_shared.js';

/** the page's `type`: 0 通用, 1 品类, 2 商品 */
...
export function toPageCouponTemplate(dto) {
  if (!dto) return {};
  ...
  return {
    ...
    type: pageScope(dto.scope),
    applicable_type: pageScope(dto.scope),
    category_id: '',
    product_id: '',
    ...
    start_time: pageDate(dto.validFrom),
    end_time: pageDate(dto.validTo),
    use_title: permanent
      ? `领取后 ${toInt(dto.validDays, 0)} 天内可用`
      : `${pageDate(dto.validFrom)} - ${pageDate(dto.validTo)}`,
...
export function toPageCouponList(dto) {
  return {
    list: mapList(dto && dto.items, toPageCouponTemplate),
...
export function toPageCouponArray(dto) {
  return mapList(dto && dto.items, toPageCouponTemplate);
}

/**
 * `GET /api/v1/coupons` → the 首页 / 微页面 coupon popup (`couponWindow`), which
 * takes `{list, image}`; the pages read
 * `data.list.length`. Only templates this shopper can still claim:
 * the popup's one button is 立即领取, and a coupon already taken is not an
 * offer. `image` would be a 弹窗 image, which the site config does not carry;
 * `couponWindow` does not render it.
 */
export function toPageCouponPopup(dto) {
  const claimable = list(dto && dto.items).filter((item) => item && item.canClaim === true);
  return { list: claimable.map(toPageCouponTemplate), image: '' };
}

/**
 * `GET /api/v1/coupons/new-user` → the 新人券 popup's `{list, image, show}`.
 *
 * `show` is meant to be 1 only on the member's first visit (`add_time ===
 ...
 * 我的优惠券.
 */
export function toPageNewUserCouponPopup(dto) {
  return { list: toPageCouponArray(dto), image: '', show: 0 };
}
...
export function toPageUserCoupon(dto) {
  if (!dto) return {};
  ...
  return {
    ...
    start_use_time: unixSeconds(dto.validFrom, 0),
    end_use_time: unixSeconds(dto.validTo, 0),
    use_title: `${pageDate(dto.validFrom)} - ${pageDate(dto.validTo)}`,
    source: text(dto.sourceKind, 'claim'),
  };
}

export function toPageUserCouponList(dto) {
  return mapList(dto && dto.items, toPageUserCoupon);
}

/** `POST /api/v1/coupons/:id/claims` → what `setCouponReceive` resolved with. */
export function toPageClaimResult(dto) {
  return {
    coupon: toPageUserCoupon(dto && dto.coupon),
 ...
 */
export function toPageApplicableCoupons(dto) {
  return mapList(dto && dto.items, (row) => ({
    ...toPageUserCoupon(row.coupon),
...
/** `getCouponsOrderPrice(price, {cartId})` → the applicable-coupon body. */
export function fromPageApplicableInput(price, data) {
  const src = data || {};
  const lines = Array.isArray(src.lines) ? src.lines : null;
  if (lines) return { lines };
  // The 确认订单 picker passes its `cartInfo` (`toPageCheckoutLine` rows):
  // one line per product, at the line's total. No
  // `categoryIds`: the server looks a line's categories up from its product
  // and ignores any the body sends.
...
/** The 我的优惠券 tab index (0 全部 / 1 未使用 / 2 已使用) → `state`. */
export function fromPageCouponState(types) {
...
// ---------------------------------------------------------------------------
// 支付成功页的「恭喜获得优惠券」 (GET /api/v1/orders/:id/gift-coupons)
// ---------------------------------------------------------------------------
 ...
 */
export function toPageGiftCoupons(dto) {
  return mapList(dto && dto.items, (item) => ({
    ...toPageUserCoupon(item),
    add_time: pageDate(item && item.validFrom),
  }));
}

// ---------------------------------------------------------------------------
// 店员赠送优惠券 (coupon.staff.contract.ts)
// ---------------------------------------------------------------------------
 ...
 */
export function fromPageStaffCouponQuery(data) {
  const src = data || {};
 ...
 */
export function toPageStaffCoupon(dto) {
  if (!dto) return {};
  ...
  return {
    ...
    // 售罄的券也列出来（让店员看得到为什么发不出去），抽屉目前不读这两个字段
    remain_count: dto.isUnlimitedSupply ? -1 : toInt(dto.remainingCount, 0),
    is_unlimited: dto.isUnlimitedSupply ? 1 : 0,
  };
}
...
/** 抽屉读 `res.data` 为裸数组。 */
export function toPageStaffCoupons(dto) {
  return mapList(dto && dto.items, toPageStaffCoupon);
}

/** 一位客户一张券：`{userId, couponId}`（单数，扇出在 api/admin.js）。 */
export function fromPageCouponGrant(uid, couponId) {
  return { userId: text(uid).trim(), couponId: text(couponId).trim() };
}

/**
 * 扇出后的若干个 `{granted, skippedUserIds}` → 抽屉 toast 的那句话。已达领取上限的
 * 客户是 200 + `skippedUserIds`，不是错误，所以要说出来，不能一律「赠送成功」。
 */
