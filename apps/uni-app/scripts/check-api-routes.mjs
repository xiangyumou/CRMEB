...
//   2. looks the method+path up in `packages/contracts/openapi.json`;
...
const OPENAPI = path.resolve(APP, '../../packages/contracts/openapi.json');
...
function loadOpenApi() {
  if (!fs.existsSync(OPENAPI)) {
    console.error(`openapi.json not found at ${OPENAPI} — run \`pnpm gen\` at the repository root first.`);
    process.exit(2);
  }
