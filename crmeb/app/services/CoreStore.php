...
final class CoreStore
{
    ...
    public static function cleanDiy($data)
    {
        ...
        static $paths;
        if ($paths === null) $paths = json_decode(file_get_contents(dirname(__DIR__, 2) . '/upgrade/core-store/removed-pages.json'), true);
        $isList = array_keys($data) === range(0, count($data) - 1);
        foreach ($data as $key => $value) {
            if (in_array((string)$key, self::REMOVED_COMPONENTS, true) ||
                (is_array($value) && in_array($value['name'] ?? '', self::REMOVED_COMPONENTS, true))) {
