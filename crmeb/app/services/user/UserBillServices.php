...
class UserBillServices extends BaseServices
{
    ...
    public function getCommissionList(array $where, int $limit = 0)
    {
        ...
        [$count, $list] = $userUserBrokerage->getBrokerageList($where_data, 'sum(IF(b.pm = 1 AND b.type <> \'extract_fail\', b.number, 0)) as income,sum(IF(b.pm = 0, b.number, 0)) as pay,u.nickname,u.phone,u.uid,u.now_money,u.brokerage_price,MAX(b.add_time) as time,MAX(b.id) as last_brokerage_id', $order_string, $limit);
