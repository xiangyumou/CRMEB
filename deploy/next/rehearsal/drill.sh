#!/usr/bin/env bash
# The deploy drill: every claim `deploy/next` makes, proved against real
...
if [ "$build" -eq 1 ]; then
  ...
  docker build -f "$repo_root/next/docker/web.Dockerfile" -t crmeb-next-web:drill "$repo_root/next" || exit 3
  docker build -f "$repo_root/next/docker/worker.Dockerfile" -t crmeb-next-worker:drill "$repo_root/next" || exit 3
  docker build -f "$repo_root/next/docker/edge/Dockerfile" -t crmeb-next-edge:drill "$repo_root" || exit 3
...
case_no_secrets_in_repo() {
  ...
  tracked="$(git -C "$repo_root" ls-files 'deploy/next' 'next/docker' 2>/dev/null || true)"
  [ -n "$tracked" ] || {
  ...
  if [ -n "$offenders" ]; then
    ...
    note 'ok: no tracked file under deploy/next or next/docker sets a password'
  fi
