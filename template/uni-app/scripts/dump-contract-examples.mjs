...
// a payload the contract itself claims is valid. Re-run it after `pnpm gen` in `next/`.
//
...
const CONTRACTS = path.resolve(APP, '../../next/packages/contracts');
const TSX = path.join(CONTRACTS, 'node_modules/.bin/tsx');
const OUT = path.join(APP, 'tests/fixtures/contract-examples.json');

if (!fs.existsSync(TSX)) {
  console.error(`tsx not found at ${TSX} — run \`pnpm install\` in next/ first.`);
  process.exit(2);
}
