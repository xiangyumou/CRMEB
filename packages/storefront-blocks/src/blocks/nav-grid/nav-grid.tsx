 ...
 */
export function NavGrid({ props, onLink }: BlockProps<NavGridProps>) {
  ...
  if (pages.length <= 1) {
    return (
      <BlockFrame type="navGrid" frame={props.style} className={styles.body}>
        <Cells items={props.items} props={props} onLink={onLink} />
      </BlockFrame>
    );
  }
  return (
      ...
      >
        {pages.map((items, index) => (
          <SwiperItem key={index}>
            <Cells items={items} props={props} onLink={onLink} />
          </SwiperItem>
        ))}
      </Swiper>
