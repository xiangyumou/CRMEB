...
/**
 * The public 装修 reads beyond the page routes: 个人中心, 底部导航, the 版式
 * switch, and 商品详情.
 *
 * ## Why these are cached
 *
 * 底部导航 is mounted on *every* tabbar page and refetched on each `onShow`,
 * 版式 is two integers behind a round trip, and 商品详情 is read on every
 * product view. Sixty seconds in Redis turns that traffic into one database
 * read a minute per surface, and every mutation that could change them drops
 * the keys, so an operator who publishes still sees the change at once. The
 * home page is cached the same way, in `diy-page.service.ts`.
 *
 ...
 *
 * The default is the stock install's default detail page
...
/**
 * `GET /api/v1/diy/navigation`.
 *
 * Read off the live home page, where the editor puts it. The component is
 * returned verbatim (after the retired-component filter every storefront read
 * applies), because `components/pageFooter/index.vue` reads two dozen fields
 * straight off it.
 *
