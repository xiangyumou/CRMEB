...
describe('storage keys are generated by the server, never by the client', () => {
  ...
  it('writes a thumbnail only next to a key it generated', async () => {
    ...
    try {
      ...
      const key = await storage.putVariant(stored.key, 750, Buffer.from('small'), 'image/jpeg');
      expect(key).toBe(stored.key.replace(/\.jpg$/, '.w750.jpg'));
      ...
      for (const bad of ['../etc/passwd.jpg', 'product/2026/02/x.jpg', `${key}`]) {
        await expect(
          storage.putVariant(bad, 360, Buffer.from('x'), 'image/jpeg'),
          bad,
        ).rejects.toThrow();
      }
      ...
      await expect(
        storage.putVariant(gif.key, 360, Buffer.from('x'), 'image/gif'),
      ).rejects.toThrow();
      ...
      const variant = await memory.putVariant(inMemory.key, 360, Buffer.from('s'), 'image/png');
