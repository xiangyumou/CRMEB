...
2. Golden fixtures in `packages/contracts/src/diy/__fixtures__/`: every default page payload from `moren.js`, plus the 6 production pages already present as `prod-{2,3,4,6,7,8}.json` (byte-exact production exports — never reformat them; they are in `.prettierignore`), plus the demo pages from the `eb_diy` INSERTs in `crmeb/public/install/crmeb.sql`. Test: parse → serialise → deep-equal, and serialise is key-order stable.
...
## Sequencing note
You start before `docs/rewrite/GOLDEN.md` exists (the coupon slice is being built in parallel). Do items 1, 2 and 5 first — they do not depend on the CRUD pattern. When you reach item 3's routes and services, check whether `docs/rewrite/GOLDEN.md` has landed on `rewrite/integration` (`git fetch` is not needed; `git log rewrite/integration -- docs/rewrite/GOLDEN.md`), merge `rewrite/integration` into your branch if so, and follow its shape; if not, model on `packages/core/src/auth` + `packages/contracts/src/auth`.
