...
## Invariants to prove
Rows under the group-buy and presale sections of `docs/rewrite/invariants.md` and risk-matrix §5. Concurrency tests (mandatory): last seat taken by two paid callbacks → one member, the other order is refunded through C; expiry racing the last join → one outcome; leader refund racing a join; presale last unit by two checkouts; cancel racing pay on a presale order leaves all four ledgers balanced.
