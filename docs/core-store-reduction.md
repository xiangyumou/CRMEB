...
- 微信支付 v2/v3 都属保留业务：`pay_wechat_type` 保留为新装种子并与其它保留配置一样参与迁移（缺失时补默认值 `0`），不随非微信支付方式一起删除。
- 会员专享券（`receive_type = 4`）随会员业务退出：前台列表、PC 列表、弹窗列表、数量统计与装修优惠券组件（`theme/coupon`）只查普通券，组件若配置为「会员用户」则直接返回空列表，按 ID 领取直接返回「该优惠券所属业务已下线」且不写任何记录；已发出的券历史记录不改写。
...
- `rollback` 恢复表名与被删除的行。它**在任何重命名之前**逐行比对备份：等于备份 `after` 状态则恢复，等于 `before` 状态视为已恢复，其它值直接以 `Concurrent changes: rollback refused` 终止且不改动数据库；`--force` 只豁免受保护表的变化，不豁免记录被编辑。`RENAME TABLE` 会隐式提交，因此改名在事务之外单独执行，之后才开新事务恢复共享表行；数据恢复失败时表名保持已恢复状态，命令提示可用**同一备份重试**，重试只补插缺失行。`finalize` 之后 `eb_retired_*` 已不存在，rollback 直接**非 0 退出**并提示需要 mysqldump 恢复，不再假成功。
- `finalize` 只删除**空**的 `eb_retired_*` 表：先统计所有待删表的行数，任一表非空就以退出码 `2` 列出表名与行数并整批不删，`--yes` 与 `--dump` 都不能绕过；空表仍需命名了全部待删表的 `--dump`（作为可恢复点的记录，不校验文件内容）并默认要求交互确认。删除非空退出表、以及从这类表恢复数据都不在本脚本能力范围内，遇到时停止并单独制定方案。
- 校验商品、规格、分类、附件、订单、订单购物车与用户表的行数和内容哈希不变。脚本不改上传文件、不删共享表字段。

维护窗口：apply 的重命名阶段按表执行且不可回滚，请安排停机窗口；期间不要同时跑定时任务或后台操作。

新装环境直接使用 `crmeb/public/install/crmeb.sql`：退出业务表、配置、配置 tab、菜单、定时任务、通知模板与自定义事件种子已移除（AllInPay 网关的设置 tab 与密钥随驱动一并删除）；保留的发票/资金流水/账单记录菜单挂回保留父级，客服配置 tab 承载 `customer_qrcode`，首页精品/热门 banner 组合数据保留，个人中心菜单选项只留下仍然存在的页面。`eb_system_route` 接口文档登记表仍保留少量指向已删控制器的行：它们是惰性文档，代码不读取，后台"同步路由"会在运行中的店铺上自行清理。`tests/static/install-sql-guard.cjs` 以迁移脚本中的退出清单为准，防止安装 SQL 再次播种这些行。`replace_site_url` 与「清除数据」共用同一份保留表清单：缺失的表跳过并记日志，不再因为退出业务的表已删除而整体失败。两者失败行为不同：「清除数据」逐表执行，单个表失败不阻断其余表并在最后汇总抛出；`replace_site_url` 把所有 UPDATE 放在同一个事务里，任一语句失败即整体回滚并抛出「替换失败,失败原因:{:msg}」，不做逐表汇总。
