...
| presale price is what the order shows, not the catalogue price                   | 部分     | `promo.spec.ts::a presale is booked and paid in full, and says when it ships` 断言 确认订单、收银台、订单行都是 ¥78；**没断言 订单详情**，而 订单详情 恰好有问题（下一行）                                                                                                                                 |
| a presale with a stacked coupon still shows the presale price on the order pages | 已知失败 | `coupons.spec.ts::a presale with a stacked coupon still shows the presale price on the order pages`（`test.fail`）。小程序 订单详情 把商品行印成目录价 ¥88.00（`unitPrice`），把活动 ¥10 和券 ¥5 合成「优惠券 -¥15.00」；uni-app 读行上的 `adjustments`，印 ¥78.00 和 ¥5.00。修好后删掉 `test.fail` 那一行 |
...
| a shopper confirms receipt from the order page and reviews it from there | 已覆盖 | 同上（确认收货 走微信确认收货组件，`via: 'wechat-component'`）                                                                    |
...
| 预售                    | `promo.spec.ts::a presale is booked and paid in full, and says when it ships`；订单页价格见 `coupons.spec.ts`（已知失败）                                                                                                                                                                                                                    |
...
2. **预售订单页价格**（已知失败）：`apps/mini` 的 订单详情 / 订单列表 要按行上的 `adjustments` 显示活动价和真正的券额。修好后删掉 `coupons.spec.ts` 里的 `test.fail`。
