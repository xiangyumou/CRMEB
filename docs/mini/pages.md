...
- 海报弹层（`PosterSheet`）和分享弹层（`ShareSheet`）放在主包的 `features/share/`，不在 `promo` 分包：商品详情（主包）和拼团进度（`promo`）都用它们，分包之间不能互相引用。海报只画商品图（可关）、名称、价格和小程序码，不画任何个人信息。
...
| ----------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `pages/index/index`     | 首页（tab，自定义导航栏）      | `pages/index/index`、`pages/guide/index`                                                                      | 新·`system.appConfigGet`、新·`decor.pageHome`、`user.recordVisit`、`cart.count`（登录后）；DIY 块的数据由 resolver 一起返回。块内的动作：`coupon.claim`、`cart.addItem`                                                                                                                                                                               | 否   | 好友+朋友圈 | 启动引导页取消，改为首页上的开屏浮层（`app/config.splashAd`），可关闭，有倒计时，每天最多一次                                                                                                                                                          |
...
| `pages/me/index`        | 我的（tab，自定义导航栏）      | `pages/user/index`                                                                                            | 新·`decor.pageUserCenter`、`user.getProfile`、`order.counts`、`notification.myUnreadCount`                                                                                                                                                                                                                                                            | 动作 | —           | 未登录时，用户卡片显示「登录/注册」，其他内容照常渲染                                                                                                                                                                                                  |
...
| `packages/order/list/index`       | 我的订单 | `pages/goods/order_list/index`        | `order.list { tab }`、`order.counts`、`order.cancel`、`order.hide`、`cart.rebuy`、`order.confirmReceipt`（通过 `platform/receipt.ts`）                                       | 是   | —    | tab 取 `orderListTab` 的值（全部、待付款、待发货、待收货、已完成、已取消；前三个带数字）；确认收货走 `platform/receipt.ts`：`payment.wechatReceipt` 有支付单时打开微信的确认收货组件（C07）并带 `{ via: 'wechat-component' }`，否则用普通确认框。下拉刷新                                                                                                                      |
| `packages/order/detail/index`     | 订单详情 | `pages/goods/order_details/index`     | `order.detail`、`order.myShipments`、`order.cancel`、`order.hide`、`order.confirmReceipt`、`payment.wechatReceipt`、`cart.rebuy`、`payment.status`（用 `outTradeNo` 打开时） | 是   | —    | 接受 `id` 或 `outTradeNo` 两种参数；`outTradeNo` 是微信发货消息的跳转方式（C07 的 `set_msg_jump_path`）。包裹卡片跳 `logistics`；「申请售后」在商品卡片上，跳到 `refundApply`；「发票」跳到 `invoiceApply`；客服按钮带订单卡片。**落地调整：** 不显示赠券（`coupon.orderGiftCoupons`）；没有「查看拼团」，因为 `order.detail` 不带团 id（第 5 节）                             |
...
| `packages/order/review/index`     | 评价商品 | `pages/goods/goods_comment_con/index` | `order.detail`、`storage.userUpload { purpose: 'review' }`、`catalog.reviewSubmit`                                                                                           | 是   | —    | 每个可评价的商品行一张卡片（商品评分、文字 500 字、最多 9 张图，用 `chooseMedia` 选择）；服务评分整单一个。**没有匿名开关**：评价本来就不存昵称和头像，页面写明「评价不显示你的昵称和头像」。逐行提交；返回 `moderation: 'pending'` 时显示「评价已提交，审核后展示」（C09）；已经评价过的行算作完成                                                                            |
...
| `packages/promo/my-coupons/index`      | 我的优惠券 | `pages/users/user_coupon/index`                                                               | `coupon.myList { state }`                                                                          | 是   | —           | 「去使用」跳到按券范围筛选的商品列表。**落地调整：** 跳 `productList { couponId }`，`couponId` 是券模板 id；商品列表接口还不能按券筛选（第 5 节）                                                                                                                                                                                                                                                                     |
...
| `packages/page/index`             | 微页面（DIY 标题） | `pages/annex/special/index`          | 新·`decor.pageResolve { id }`                  | 否   | 好友+朋友圈（可在 DIY 里关闭） | 旧的 `diy.page` 只返回装修 JSON，新的返回 resolve 之后的块数据；分享标题和图片来自 `root.props` |
...
| `shipping.expressCompanyOptions`                                                                         | `GET /api/v1/express-companies`                                            | public（已由 H2 实现） | 退货物流选择快递公司（修复旧页面的空列表）                                                                                                                                                                                                                                                                                                                                                                                                    | H1  |
