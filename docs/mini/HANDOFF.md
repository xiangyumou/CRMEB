...
- `master` 等于线上，只修线上紧急问题。开发不在 `master` 上做。`master` 到 `508f009` 为止的提交都已并入 `storefront/mini`。
...
2. **K1 的 B1（会话续期换了账号还重放请求）。** 这是本分支的中危问题：401 续期用 `wx.login` 登到持有这个 openid 的账号，再以那个账号重放失败的请求（写也会重放）。H6 修好了常见情况；openid 属于别的账号时还在。建议：续期后账号变了就不重放，回到登录页。改动在 `apps/mini/src/session/renewing-transport.ts`、`session.ts`，加单测和一条 e2e。
...
1. **K1 P1（kindMeta 价格绕过）：** `master` 上已有 `508f009` 的临时修法（`kindMeta` 先展开，`kind` 和优惠券后写）。它部署到线上了吗？没部署的话，外网提前开放前要部署，或者随切换上线。
2. **K1 P5 / B3：** 拼团团页是否隐藏团员 `userId`、给昵称打码；微信内容安全不可用时昵称是放行还是拒绝。
3. **K1 B2：** 没有小程序 openid 的账号（任何人都能通过短信登录注册）发的评价，是否一律进待审核。切换前这会把所有旧 H5 顾客的评价都送去人工审核。
4. **K1 B4：** 消息推送是否在代码里强制安全模式，或者 Redis 不可用时拒绝明文 / 兼容模式的推送。
5. **H6：** 绑定被拒后不带 bindToken 重试、静默登录成功，要不要加一条提示。
6. **K3：** 我的每次打开都发两个请求（页面 + 未读数，旧 uni-app 也这样），可以接受吗，还是只在标记过期时刷新？文案按领券中心统一，还是反过来？
7. **K2：** 分包预算 1 MB 还是 2 MB；没有代码用 `@nutui/nutui-react-taro`，只剩 2 KB 的配色桥接，保留还是删掉。
8. **R1：**
...
9. **切换方式：** 一次发布，还是 I2 提的三次发布（A：新后端不删东西，先送审；B：删除旧代码、上落地页；C：DROP 旧表）。
10. 公众号模板消息的链接是否改为打开小程序（在 B 之前）。
11. 装修模板里「所有订单均以隐私包装发出，外包装不显示商品信息」这句承诺是否属实。
12. 界面配色：默认品牌色 #E1251B，建议换成更克制的配色。截图在 `docs/mini/status/*-screens/`。
13. 谁做真机测试。
14. master 目录里那 6 个多余文件要不要删。
