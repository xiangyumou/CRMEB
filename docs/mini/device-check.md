...
e2e（`pnpm --filter @shop/e2e-storefront test:mini`）跑的是「模拟小程序」H5 构建，看不到 WXML、原生组件、
...
- **AppSecret 只存在服务端配置里**：生产服务器，或者你本机后台「小程序」配置分组（`wechat-mini`，
  ...
  **不要为了测试去公众平台「重置」AppSecret**：重置后旧的立即失效，生产环境的登录会马上断掉。
...
| **B. e2e 全栈（全是假件）** | `SHOP_E2E_CLIENT=mini pnpm --filter @shop/e2e-storefront serve`（需要 Docker），日志里 `starting edge on :<端口>` | 真实服务端逻辑和种子数据，匿名浏览首页、分类、商品。                                                                     | **登录**：真机和开发者工具里 `wx.login` 拿到的是微信真实的 code，假的 `api.weixin.qq.com` 只认种子里的 code，回 `40029 invalid code`。支付走假网关，真机上付不了。 |
...
- A 最省事，UI 检查首选。B 用来看真实数据下的页面。C 会从本机访问 `api.weixin.qq.com`，每次启用都先征得
  你本人同意；AppSecret 只能填你已经掌握的那个，不要重置（见第 1 节）。手机号快捷登录按次计费。
...
| D02 | tabBar               | 四个 tab 来回切；加购一件商品后看购物车 tab                                                      | 文字、颜色、选中色和后台主题一致（运行时设置）；购物车角标数字正确，清空后消失                                                                                               | A/B    |
| D03 | 隐私弹窗             | 第一次触发隐私接口（手机号按钮、选择收货地址）                                                   | 先弹隐私协议，同意后才继续；拒绝后有提示、不卡死（C04）                                                                                                                      | A/B/C  |
| D04 | 静默登录             | 冷启动；「真机调试」里看 Network                                                                 | 没有登录弹窗；有一次 `POST /api/v1/auth/sessions/wechat-mini`；热启动（切后台再回来）不再调用。后端 B 这里失败（40029）是预期，记下来即可                                    | A/C    |
| D05 | 手机号按钮           | 新用户（`phone-required`）点「手机号快捷登录」，分别允许和拒绝                                   | 允许 → 登录成功；拒绝 → 提示，按钮还能再点                                                                                                                                   | C      |
